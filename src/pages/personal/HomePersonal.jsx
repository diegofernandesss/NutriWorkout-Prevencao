...
export const HomePersonal = () => {
  ...
  const [atletaPersonal, setAtletaPersonal] = useState([]);
  const [activePage, setActivePage] = useState(1);
  const [totAtletaPersonal, setTotAtletaPersonal] = useState(0)
  ...
  const getAtletaPersonal = async () => {
    try {
      const response = await api.get(`/personal/atleta/${activePage}/${max_items}`);
      setAtletaPersonal(response.data.atletasPersonal);
      setTotAtletaPersonal(response.data.totalAtletas);
    } catch (error) {
      console.error(error);
    } 
  };
  ...
  return (
    ...
    {atletaPersonal.length === 0 && (
          <div className='flex justify-center items-center mt-5'>
            ...
            totalItemsCount={totAtletaPersonal}
