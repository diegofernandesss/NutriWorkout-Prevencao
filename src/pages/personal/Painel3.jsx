...
export const Painel3 = () => {
    ...
    const [notificacoesPersonal, setNotificacoesPersonal] = useState([]);
    ...
    const toggleDropdown = async () => {
        if(!isDropdownOpen){
            ...
            const response = await api.get("/personalTrainer/notificacoes")
            setNotificacoesPersonal(response.data)
        }
    ...
    const aceitarOuRecusarPedido = async (id, situacao) => {
        if (id) {
            ...
            try{
                const response = await api.patch("/personalTrainer/notificacao", information)
                notifySuccess(response)
                setNotificacoesPersonal((prevData) => prevData.filter((notificacao) => notificacao.id !== id));
            } catch (error) {
                notifyError(error)
            }
        }
    ...
    useEffect(() => {
        const personal = async () => {
            try {
                const response = await api.get(`/personalTrainer/${id}`);
                setPersonalTrainer(response.data.personal);
            } catch (error) {
                console.error(error);
            } 
            }
                    ...
                    <li className="mr-4 relative">
                        ...
                        <div className="absolute mt-5 right-0 z-10 w-96 origin-top-right rounded-md bg-gray-800 py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                            <div className="mt-3 mb-2">
                                ...
                                {notificacoesPersonal.map((notification) => (
                        ...
                        <ul className="absolute mt-3 right-0 z-10 w-80 origin-top-right rounded-md bg-gray-800 py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none ">
                            <li>
                                <a href="#" className="py-2 px-4 rounded-md flex items-center hover:bg-gray-600 group">
                                    ...
                                    />
                                    <div className="ml-2">
                                        <div className="text-[15px] text-gray-100 font-medium">{personalTrainer.nome}</div>
                                        <div className="text-[13px] text-gray-300">{personalTrainer.email}</div>
                                    </div>
                                </a>
                            </li>
                            <li>
